...
place: sending real mail through a Gmail account via SMTP, plus the
...
Gmail setup required (see .env.example):
    1. The Gmail account must have 2-Step Verification turned on.
    2. Create an "App Password" for it (Google Account -> Security ->
       2-Step Verification -> App passwords) — a 16-character password
       used only for this app, separate from the normal login password.
    3. Put the Gmail address and that app password in .env as
       GMAIL_ADDRESS / GMAIL_APP_PASSWORD.

Nothing here depends on any third-party mail library — smtplib and
the email.mime.* classes are both in the Python standard library, so
no new entry in requirements.txt is needed.
"""
...
def _send_email(to_address: str, subject: str, html_body: str, text_body: str | None = None) -> bool:
    """Sends one email through the configured Gmail account over SMTP
    (SSL, port 465 by default — see app/config.py's MAIL_SMTP_* /
    GMAIL_* settings). Returns True on success, False on any failure
    (bad/missing credentials, network error, etc.) instead of raising,
    so a mail hiccup never turns into a 500 for the applicant — callers
    decide what a failed send should mean for their own flow (e.g. the
    ...
    Returns False immediately, without attempting to connect, if
    GMAIL_ADDRESS/GMAIL_APP_PASSWORD haven't been configured — this is
    what happens out of the box in local dev before .env is filled in.
    """
    ...
    if not gmail_address or not gmail_app_password:
        current_app.logger.warning(
            "email_utils: GMAIL_ADDRESS/GMAIL_APP_PASSWORD not configured — "
    ...
    host = current_app.config.get("MAIL_SMTP_HOST", "smtp.gmail.com")
    port = current_app.config.get("MAIL_SMTP_PORT", 465)
    ...
    try:
        ...
        return True
    except Exception:  # noqa: BLE001 — any SMTP/network failure, logged and swallowed
        current_app.logger.exception(
            "email_utils: failed to send %r to %s", subject, to_address
        )
        return False
...
def send_verification_code(email: str, purpose: str = "registration") -> bool:
    ...
    Gmail SMTP. Returns whatever `_send_email` returned — True if the
    send succeeded, False otherwise. The code row is still saved even
    if sending fails, so config/logging is consistent, but callers
    should treat a False return as "could not deliver" server-side.
    """
    code = _generate_code()
